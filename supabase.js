...
const DB = {
  ...
  async saveSettings(data) {
    return this.upsert('settings', { id: 1, ...data });
  },
  ...
  async saveScoresBulk(scoreRows) {
    ...
    return this.upsert('scores', clean);
  },
  ...
  async saveAttendanceBulk(records) {
    return this.upsert('attendance', records);
  },
  ...
  async savePsychomotorBulk(rows, status) {
    ...
    return this.upsert('psychomotor', tagged);
  },
  ...
  async getPendingPsychomotor() {
    return this.selectRaw('psychomotor', 'status=eq.submitted&order=class_name.asc');
  },
  ...
  async getNotices(limit) {
    return this.selectRaw('notices', `order=created_at.desc&limit=${limit || 5}`);
  },
  ...
  async getActivityLog(limit) {
    return this.selectRaw('activity_log', `order=created_at.desc&limit=${limit || 20}`);
  },
